...
void I_FinishUpdate(void) {
    ...
    // Draw little dots on the bottom of the screen (debug mode)
    if (devparm) {
        i = I_GetTime();
        tics = i - lastTic;
        lastTic = i;
    ...
    // Scale the screen size before blitting it
    if (multiply == 2) {
        scaleScreen(2);
    } else if (multiply == 3) {
        scaleScreen(3);
...
void scaleScreen(int scaleFactor) {
    unsigned int *ilineptr;
    unsigned int *olineptrs[scaleFactor];
    int x, y, i;
    unsigned int pixels[scaleFactor];
    ...
    while (y--) {
        ...
        do {
            ...
            for (i = 0; i < scaleFactor; i++) {
                *olineptrs[i]++ = pixels[i];
            }
#else
            for (i = scaleFactor - 1; i >= 0; i--) {
                *olineptrs[i]++ = pixels[i];
            }
#endif
        } while (x -= 4);
        for (i = 0; i < scaleFactor; i++) {
            olineptrs[i] += X_width / 4;
        }
    }
}

void updateXShm(void) {
